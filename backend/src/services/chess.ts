...
export class ChessService {
  ...
  async list(a: Actor) {
    return this.core.db.query(
      'SELECT id,white_team,black_team,fen,ply,status,result,termination,score_void,created_at,completed_at FROM chess_matches WHERE white_team=$1 OR black_team=$1 ORDER BY created_at DESC LIMIT 100',
      [a.team_id],
    );
  }
    ...
    return this.core.db.tx(async (q) => {
      ...
      return this.core.operation(q, a, op, ['matchmaking'], async () => {
        ...
        const [m] = await q.query(
          'INSERT INTO chess_matches(id,white_team,black_team,initial_fen,fen,scoring_snapshot) VALUES($1,$2,$3,$4,$4,$5) RETURNING *',
          [id, white, black, DEFAULT_POSITION, JSON.stringify(s.scoring)],
        );
        await q.query(
        ...
        await this.core.audit(q, a, 'CHESS_MATCH_CREATED', id, { white, black });
        await this.emit(q, m, 'chess:match_found', { matchId: id, status: 'MATCHED' });
    ...
    return this.core.db.tx(async (q) => {
      ...
      return this.core.operation(
        ...
        async () => {
          ...
          await q.query('UPDATE chess_matches SET fen=$2,ply=ply+1 WHERE id=$1', [id, game.fen()]);
          if (played.captured) {
          ...
          return { matchId: id, move, fen: game.fen(), completed: game.isGameOver() };
        },
      );
    });
  }
